...
const MONTHS: MonthBlock[] = [
  {
    month: "January 2026",
    events: [{ name: "The Tree Farm Invitational", month: "January 2026" }],
  },
  {
    month: "February 2026",
    events: [{ name: "Gasparilla Invitational", month: "February 2026" }],
  },
  {
    month: "March 2026",
    events: [
  ...
  },
  {
    month: "August 2026",
    events: [
      { name: "The Picard Cup", month: "August 2026" },
      { name: "The Valentine Invitational", month: "August 2026" },
    ],
  },
  {
...
export const metadata = {
  title: "Mid-Am Majors (Mock A) | midamgolfHQ",
  description: "Mock A: month sections with compact rows.",
};

export default function MidAmMajorsMockA() {
  return (
    <SiteShell>
      <div className="mx-auto w-full max-w-4xl px-5 py-8">
        <div className="mb-6">
          <div className="text-xs font-medium text-zinc-500">Mock A</div>
          <h1 className="mt-2 font-serif text-4xl font-semibold tracking-tight text-zinc-950">Mid-Am Majors</h1>
          <p className="mt-3 text-sm leading-relaxed text-zinc-600">Grouped by month. Rows-only. Event + month only.</p>
        </div>

        <div className="space-y-8">
          {MONTHS.map((m) => (
            <section key={m.month} className="">
              <h2 className="text-xs font-semibold uppercase tracking-wide text-zinc-500">{m.month}</h2>
              <div className="mt-3 divide-y divide-zinc-200 rounded-3xl border border-zinc-200 bg-white">
                {m.events.map((e) => (
                  <Link
                    key={e.name}
                  ...
                  >
                    <div className="flex items-baseline justify-between gap-4">
                      <div className="font-serif text-xl font-semibold tracking-tight text-zinc-950">{e.name}</div>
                      <div className="text-xs font-medium text-zinc-500">{e.month}</div>
                    </div>
