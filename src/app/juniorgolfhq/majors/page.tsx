...
export const metadata = {
  title: "Junior Majors — 2026 | juniorgolfHQ",
  description: "Junior majors schedule for 2026: event hubs and year-by-year results pages.",
};

export default function JuniorMajorsIndexPage() {
    ...
    <SiteShell brandSlug="juniorgolfhq">
      <div className="mx-auto w-full max-w-5xl px-5 py-8">
        <div className="mb-8 text-center">
          <h1 className="font-serif text-4xl font-semibold tracking-tight text-zinc-950">Junior Majors — 2026</h1>
        </div>
          ...
          {months.map((m) => (
            <section key={m.monthKey} id={m.monthKey} className="scroll-mt-24 rounded-3xl border border-zinc-200 bg-white p-6">
              ...
              <div className="mt-4 divide-y divide-zinc-200/70">
                {m.events.map((e) => (
                  <Link key={e.slug} href={`/juniorgolfhq/majors/${e.slug}`} className="block px-1 transition hover:bg-zinc-50">
                    <div className="py-3">
                      ...
                      {e.winners2026 ? (
                        <div className="mt-1 text-xs text-zinc-600">
                          2026 winners: {e.winners2026.boys ? `Boys — ${e.winners2026.boys}` : ""}
                          {e.winners2026.girls ? ` • Girls — ${e.winners2026.girls}` : ""}
                        </div>
                      ) : null}
                    </div>
                  </Link>
